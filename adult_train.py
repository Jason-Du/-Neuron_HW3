...
def read_data(data_dirname_path):
    ...
    Trainx =Trainx.reshape(-1,106,1)
    # Vaildx = Vaildx.reshape(-1,166, 586, 1)
    Testx  = Testx.reshape(-1,106,1)
...
def RBF():
    model = Sequential()
    model.add(Dense(106, input_dim=(106)))
    model.add(RBFLayer(2, 0.5))
...
def NN():
    ...
    model.add(Dense(2, input_dim=(106), activation='relu'))
...
if __name__ == "__main__":
    Trainx, Trainy,Testx, Testy=read_data(os.path.join(os.path.dirname(__file__),'adult_dataset'))
    print(Trainy.shape)
    model_name='weight'
    # model_name='NN'
    if model_name=='RBF':
        Trainx = Trainx.reshape(Trainx.shape[0], (106))
        Testx = Testx.reshape(Testx.shape[0], (106))
        # print(Trainx.shape)
        # os.system('pause')
        model = RBF()
        model.compile(optimizer=Adam(lr=0.00025, beta_1=0.9, beta_2=0.999, epsilon=1e-08),loss='binary_crossentropy', metrics=['accuracy'])
        History = model.fit(Trainx, Trainy, batch_size=300, epochs=15, verbose=2,validation_data=(Testx,Testy))
        ...
        plt.show()
    if model_name=='NN':
        checkpoint_path = "adult_model/cp_{epoch:04d}.hdf5"
        ...
        # os.system('pause')
        model = NN()
        model.compile(optimizer=Adam(lr=0.0008, beta_1=0.9, beta_2=0.999, epsilon=1e-08),loss='binary_crossentropy', metrics=['accuracy'])
        History = model.fit(Trainx, Trainy, batch_size=300, epochs=40, verbose=2,validation_data=(Testx,Testy),callbacks=[checkpoint])
        pre = model.evaluate(Testx, Testy, batch_size=100, verbose=2)
        print('test_loss:', pre[0], '- test_acc:', pre[1])
    ...
    if model_name=='weight':
        ...
        path=os.path.join(os.path.dirname(__file__),'adult_model')
        ...
        plt.title("The Title")
        x=[i+1 for i in range(epoch_bias.shape[0])]
        for line_num in range(epoch_bias.shape[1]):
            single_weight=epoch_bias[:,line_num]
            print(single_weight)
