...
def read_data(data_dirname_path):
    ...
    Trainx =Trainx.reshape(-1,11,1)
    # Vaildx = Vaildx.reshape(-1,166, 586, 1)
    Testx  = Testx.reshape(-1,11,1)
...
if __name__ == "__main__":
    Trainx, Trainy,Testx, Testy=read_data(os.path.join(os.path.dirname(__file__),'bike_dataset'))
    # print(Trainy.shape)
    # model_name='mnist_weight'
    model_name='RBF'
    if model_name=='RBF':
        Trainx = Trainx.reshape(Trainx.shape[0], (11))
        Testx = Testx.reshape(Testx.shape[0], (11))
        # print(Trainx.shape)
        # os.system('pause')
        model = RBF()
        model.compile(optimizer=Adam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-08),loss='mse', metrics=['mae'])
        History = model.fit(Trainx, Trainy, batch_size=4, epochs=50, verbose=2,validation_data=(Testx,Testy))
        pre = model.evaluate(Testx, Testy, batch_size=4, verbose=2)
    ...
    if model_name=='NN':
        ...
        model = NN()
        model.compile(optimizer=Adam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-08),loss='mse', metrics=['mae'])
        History = model.fit(Trainx, Trainy, batch_size=4, epochs=50, verbose=2,validation_data=(Testx,Testy),callbacks=[checkpoint])
        pre = model.evaluate(Testx, Testy, batch_size=4, verbose=2)
        print('test_loss:', pre[0], '- test_acc:', pre[1])
    ...
    if model_name == 'weight':
        path = os.path.join(os.path.dirname(__file__), 'bike_model')
